...
public class Demo : MonoBehaviour
{
    ...
    private int _desiredPlayerUnitCount = 30;
    ...
    */
    ...
    /*
    private List<GameObject> _hiddenPlayers = new List<GameObject>();
    ...
    */
    ...
    {
        //
        // For testing, this "enables/disables" all player units
        //
        ...
        if (Input.GetKeyUp(KeyCode.K))
        {
            ...
            {
                playersHidden = true;
                var players = FindObjectsOfType<FoWPlayerUnit>();
                foreach (var p in players)
                {
                    if (p != null)
                    {
                        _hiddenPlayers.Add(p.gameObject);
    ...
    private void FindUnoccupiedTile(out int x, out int z)
    {
        do
        {
            x = UnityEngine.Random.Range(0, SizeX);
            z = UnityEngine.Random.Range(0, SizeZ);
    ...
    {
        RaycastHit hitInfo;
        if (Physics.Raycast(new Vector3(x, 30, z), Vector3.down, out hitInfo, 40f, 1 << 8)) // LayerMask.NameToLayer("Terrain")))
        {
    ...
    private DemoChunk GetChunkForTile(int x, int z)
    {
        ...
        if (chunk == null)
        {
            ...
            chunk.GameObject.layer = 8; //LayerMask.NameToLayer("Terrain");
