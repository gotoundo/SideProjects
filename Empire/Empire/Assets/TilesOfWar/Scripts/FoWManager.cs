...
public class FoWManager : MonoBehaviour
{
    ...
    public VisibilityType Visibility = VisibilityType.Circle;
    ...
    {
        get { return _viewerBag ?? (_viewerBag = new GameObjectBag()); }
    }
    ...
    {
        return _instance ?? (_instance = FindObjectOfType<FoWManager>());
    }
        ...
        if (projector != null && FogType == FoWType.Projector)
        {
            ...
            if (_projector != null)
            {
                ...
                transform.position = new Vector3(
                    WorldMin.x + ((WorldMax.x - WorldMin.x) * 0.5f) - (WorldUnitsPerTileSide * 0.5f),
                    WorldMax.y + 5,
                    WorldMin.z + ((WorldMax.z - WorldMin.z) * 0.5f) - (WorldUnitsPerTileSide * 0.5f)
                    );
            }
        }
    ...
    {
        for (var i = 0; i < Viewers.Items.Count; i++)
        {
            var viewer = Viewers.Items.ElementAt(i);
            if (viewer == null)
                continue;
    ...
    private void SetVisible(byte viewRadiusInTiles, short tileX, short tileZ)
    {
        ...
        for (var dx = -(viewRadiusInTiles); dx <= viewRadiusInTiles; dx++)
        {
            for (var dz = -(viewRadiusInTiles); dz <= viewRadiusInTiles; dz++)
            {
                ...
                if (ConsiderForVisibility(viewRadiusInTiles, dx, dz))
                {
                    if (TileIsVisible(tileX, tileZ, px, pz))
                    {
                        if (FadeVisibility)
                        {
                            ...
                            var lightValue = CalculateLightValue(modsqrmag, dist);
    ...
    private static float CalculateLightValue(float modsqrmag, int dist)
    {
        return Mathf.Lerp(0f, 1f, 1f - (modsqrmag / dist));
    }

    private bool ConsiderForVisibility(byte viewRadiusInTiles, int dx, int dz)
    {
        switch (Visibility)
        {
            default:
            case VisibilityType.Circle:
                var dist = viewRadiusInTiles * viewRadiusInTiles;
                var v = new Vector2(dx, dz);
                var sqrmag = v.sqrMagnitude;
                var modsqrmag = sqrmag - (WorldUnitsPerTileSide * 0.5f);
                return modsqrmag <= dist;

            case VisibilityType.Diamond:
                return ((Mathf.Abs(dx) + Mathf.Abs(dz)) <= viewRadiusInTiles);

            case VisibilityType.Square:
                return true;
        }
    }
